...
void main() {
  group('WalletService - Testes Unitários', () {
    ...
    test('getUserMovements - Deve mapear o histórico de movimentações paginadas com sucesso', () async {
      ...
      // Act
      final result = await WalletService.getUserMovements(limit: 10, lastMovementId: 'mov_init');
    ...
    test('getPortfolioValuation - Deve converter os pontos gráficos e variação da carteira com sucesso', () async {
    ...
    test('deposit - Deve processar um depósito e retornar o novo saldo do usuário', () async {
      // Payload de resposta imitando WalletTransactionResponse
      final tTransactionPayload = {
        'result': {
          'userId': 'user_allan_99',
          'newBalance': 35000.0,
        }
      };
    ...
    test('withdraw - Deve processar um saque com sucesso e atualizar o saldo', () async {
      final tTransactionPayload = {
        'result': {
          'userId': 'user_allan_99',
          'newBalance': 10000.0,
    ...
    test('withdraw - Deve propagar erro amigável em caso de falha controlada na Cloud Function', () async {
      ...
      final fakeCallable = FakeHttpsCallable(resultToReturn: FakeHttpsCallableResult(null), exceptionToThrow: exception);
      BaseService.testFunctionsInstance = FakeFirebaseFunctions(fakeCallable);
      ...
      expect(result.errorCode, 'insufficient-funds'); // Verificando extração dinâmica e detalhada do BaseService
      expect(result.message, 'Saldo em conta insuficiente para concluir o saque.');
    });
  });
}