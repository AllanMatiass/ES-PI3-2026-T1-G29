...
void main() {
  group('OfferService - Testes Unitários', () {
    ...
    final Map<String, dynamic> tOfferItemPayload = {
      ...
      'seller': {'id': 'user_roberto_77', 'name': 'Roberto', 'type': 'USER'},
      'qtdTokens': 100,
    ...
    test(
      'getOffers - deve obter a lista global de ofertas paginadas com sucesso',
      () async {
    ...
    test(
      'getMyOffers - deve retornar a lista de ofertas criadas pelo usuario logado',
      () async {
        final tResponsePayload = {
          'result': {
            'offers': [tOfferItemPayload],
    ...
    test(
      'createOffer - deve criar uma nova oferta enviando a data no formato iso8601',
      () async {
        final fakeResult = FakeHttpsCallableResult({
          'result': tOfferItemPayload,
        });
        final fakeCallable = FakeHttpsCallable(resultToReturn: fakeResult);
        BaseService.testFunctionsInstance = FakeFirebaseFunctions(fakeCallable);
    ...
    test(
      'acceptOffer - deve aceitar uma oferta e retornar o mapa com os dados da transacao',
      () async {
    ...
    test(
      'isOfferExpired - deve validar mapeamento booleano quando a oferta estiver expirada',
      () async {
        ...
        // act
        final result = await OfferService.isOfferExpired(
          offerId: 'offer_abc123',
        );
    ...
    test(
      'cancelOffer - deve validar mapeamento booleano quando a oferta for cancelada',
      () async {
