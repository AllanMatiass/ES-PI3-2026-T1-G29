...
test('deve lançar erro se startup não existir', async () => {
    ...
    await expect(service.calculateRisk('1'))
        .rejects
        .toThrow('Startup não encontrada.');
});
...
test('deve classificar corretamente o perfil de risco', () => {
    ...
    expect(service.getRiskProfile(2)).toBe('lowRisk');
    expect(service.getRiskProfile(5)).toBe('mediumRisk');
    expect(service.getRiskProfile(9)).toBe('highRisk');
});